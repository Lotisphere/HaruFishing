...
*   **模块 C: 裁判生成器 (`core/synthesizer.py`)**
    ...
    *   **社交网络量化审判 (SNA)：** 自动推算每个核心角色的有效传播率、扩散半径换算等硬核指标，并强制大模型展示计算逻辑。

## 🌐 可视化与 UI
前端基于 **Streamlit** (`ui/web_app.py`) 构建，包含以下特色：
- **无缝三语界面：** 瞬间无缝切换 English、中文和日本語。
- **赛博极客美学：** 摒弃所有圆角的绝对直角黑白设计，底层注入基于 SVG 分形算法的随机等高线暗纹背景，全局搭配「Shippori Mincho」老式明朝体，氛围感拉满。
- **全局实时监控看板：** 动态图谱自带悬浮面板，实时通过算法推算当前局势的网络密度、集群系数和模块化程度。
